...
function getCategoryBadgeClass(category: string) {
  ...
  if (category === "Custom Override") {
    return "text-amber-300 bg-amber-500/10 border-amber-500/25";
  }
  if (category.includes("AI") || category.includes("Mentor") || category.includes("Directive")) {
    return "text-fuchsia-300 bg-fuchsia-500/15 border-fuchsia-500/30";
  }
...
export function TaskCheckinCard({
  ...
  // Synchronize when parent passes fresh initialTasks from dashboard cache
  useEffect(() => {
  ...
  const handleRegenerateTasks = async () => {
    try {
      ...
      setTasks(json.data || []);
      toast.success(json.message || "Tasks aligned with active roadmap!");
    } catch (err: any) {
        ...
        <div className="flex items-center gap-2">
          {!loadingTasks && tasks.length > 0 && (
            <button
              ...
              title="Re-align protocol with active roadmap"
              className="p-1 rounded-lg hover:bg-white/5 text-white/40 hover:text-cyan-300 transition-colors disabled:opacity-50"
            >
              <RefreshCw
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[200px] pr-2 custom-scrollbar">
              {tasks.length === 0 && !isAdding && (
                <p className="text-white/40 text-sm text-center py-4">
                  Protocol is empty. Add a task or sync with roadmap.
                </p>
              )}
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span
                      className={`inline-block text-[10px] px-2 py-0.5 rounded-full border mt-1.5 font-semibold tracking-wide ${getCategoryBadgeClass(task.category)}`}
                    >
                      {task.category}
                    </span>
                  </div>
          ...
          {!loadingTasks && !isAdding && (
            <div className="mt-3 flex items-center gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 border border-dashed border-white/10 hover:border-white/20 rounded-xl text-white/50 hover:text-white text-xs font-semibold uppercase tracking-wider transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-3 py-2 border border-white/10 hover:border-white/20 bg-white/[0.02] hover:bg-white/5 rounded-xl text-slate-400 hover:text-cyan-300 text-xs font-medium transition-colors disabled:opacity-50"
              >
                <RefreshCw
                  className={`w-3.5 h-3.5 ${isRegenerating ? "animate-spin text-cyan-400" : ""}`}
                />
