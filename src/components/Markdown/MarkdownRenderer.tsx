    ...
    >
      <ReactMarkdown
        ...
        components={{
          ...
          // H2 — Section heading with divider
          h2: ({ children, ...props }) => (
            <>
              <hr className={styles["heading-divider"]} />
          ...
          // H3 — Normal subsection (NO DIVIDER)
          h3: ({ children, ...props }) => (
          ...
          li: ({ node, ordered, children, ...props }) => (
            <li
              className={
                ordered ? styles["li-ordered"] : styles["li-unordered"]
              }
              {...props}
            >
              {children}
          ...
          code: ({ node, inline, className, children, ...props }) => {
            ...
            // 1) SPECIAL FIX:
            // If it's NOT a fenced block AND appears alone on its own line,
            // treat it as inline code, not a block.
            const isSingleInline = inline || (!className && text.length < 50);
            ...
            // 2) Otherwise handle as real multi-line code block
      ...
      {abbreviations.length > 0 && (
        <div className={styles["abbr-legend"]}>
          <h4 className={styles["abbr-legend-title"]}>Glossary</h4>
          <dl className={styles["abbr-legend-list"]}>
            {abbreviations.map((item, idx) => (
              <div key={idx} className={styles["abbr-legend-item"]}>
                ...
                <dd className={styles["abbr-legend-definition"]}>
                  {item.full}
                </dd>
              </div>
            ))}
